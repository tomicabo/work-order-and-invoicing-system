...
namespace POSSavkovic.Views
{
    ...
    public partial class NatisniRacun : Window
    {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            list_items.Cena_z_ddv = (Convert.ToDecimal(reader["cena_z_ddv"])).ToString("#,###.00");
                            list_items._Cena_z_ddv = (Convert.ToDecimal(reader["cena_z_ddv"]));
                        }
                        ...
                        {
                            list_items.Ddv = (Convert.ToDecimal(reader["ddv"])).ToString("#,###.00");
                            list_items._Ddv = (Convert.ToDecimal(reader["ddv"]));
                        }
        ...
        private void Izracun()
        {
            ...
            decimal izracun_ddv = 0;
            decimal izracun_vrednost = 0;

            for (int i = 0; i < lvi_list.Count; i++)
            {
                izracun_skupaj_brez_ddv = izracun_skupaj_brez_ddv + lvi_list[i]._Vrednost;
                izracun_ddv = izracun_ddv + (lvi_list[i]._Cena_z_ddv - lvi_list[i]._Vrednost);
                izracun_vrednost = izracun_skupaj_brez_ddv + izracun_ddv;
            }

            tb_od_zneska.Text = izracun_skupaj_brez_ddv.ToString();
            tb_skupaj.Text = izracun_skupaj_brez_ddv.ToString("#,###.00 €");
            tb_ddv.Text = izracun_ddv.ToString("#,###.00 €");
            tb_za_placilo.Text = izracun_vrednost.ToString("#,###.00 €");
        }
